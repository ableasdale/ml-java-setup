...
import com.marklogic.support.exceptions.ConnectionFailedException;
import com.marklogic.support.exceptions.ContentSourceAlreadyEnlistedException;
...
public class LoadBalancedMarkLogicContentSource {
     ...
     */
    private LoadBalancedMarkLogicContentSource() {
        LOG.info(MessageFormat.format(
                "Constructor :: Creating LoadBalancedMarkLogicContentSource at {0}",
                new Date()));
        ...
        List<String> hosts = Arrays.asList(Util.getConfiguration().getStringArray("hosts"));
        ...
        try {
            ...
            LOG.error("Required Parameters are null when initializing, unable to continue", e);
        } catch (ConfigurationException e) {
            LOG.error("ConfigurationException encountered: ",e);
        }
    }
    ...
    private void init(List<String> uriList) throws ConfigurationException,
        ...
        for (String s : uriList) {
            try {
                ...
                LOG.error("ContentSourceAlreadyEnlistedException encountered: ",e);
            }
        }
        ...
        if (activeContentSourceList.size() == 0) {
            ...
            // TODO - we need to find a way to ensure the LoadBalancedMarkLogicContentSource
     ...
     *
     * @param contentSource The ContentSource Object for delisting.
     */
     ...
     *
     * @param session The Session for validation
     ...
     * timestamp
     *
     * @param contentSource The ContentSource for validation
     * @return true or false
     */
     ...
     *
     * @param s The Session that caused connectivity issues.
     */
    ...
    private synchronized Session rotateContentSourceListAndOpenSession() {
        ...
        try {
            ...
            LOG.error("ConnectionFailedException encountered: ",e);
    ...
    public List<ContentSource> getCopyOfActiveContentSourceList() {
        return new ArrayList<>(activeContentSourceList);
    }
